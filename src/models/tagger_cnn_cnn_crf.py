...
from src.layers.layer_cnn import LayerCNN


class TaggerCNNCNNCRF(TaggerBase):
    """TaggerCNNCNNCRF is a model for sequences tagging that includes conv network + conv layer + CRF."""
    def __init__(self, word_seq_indexer, tag_seq_indexer, class_num, batch_size=1,
                 freeze_word_embeddings=False, dropout_ratio=0.5, gpu=-1,
                 freeze_char_embeddings = False, char_embeddings_dim=25,
                 word_len=20, char_cnn_filter_num=30,
                 char_window_size=3, cnn_window_size=5):
        super(TaggerCNNCNNCRF, self).__init__(word_seq_indexer, tag_seq_indexer, gpu, batch_size)
        ...
        self.cnn_layer = LayerCNN(gpu, self.word_embeddings_layer.output_dim +
                                  self.char_cnn_layer.output_dim,
                                  self.word_embeddings_layer.output_dim +
                                  self.char_cnn_layer.output_dim,
                                  self.cnn_window_size)

        self.lin_layer = nn.Linear(in_features=self.cnn_layer.output_dim +
                                   self.word_embeddings_layer.output_dim +
                                   self.char_cnn_layer.output_dim,
    ...
    def _forward_birnn(self, word_sequences):
        ...
        # here we apply our cnn
        cnn_out = self.apply_mask(self.cnn_layer(z), mask)
        # after concat it with z
        concated_cnn_out = torch.cat((cnn_out, z), dim=2)
        features_rnn_compressed = self.lin_layer(concated_cnn_out)
